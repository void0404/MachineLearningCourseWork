...
#异常值
train_data.drop(train_data[train_data["debt_loan_ratio"] > 42.88].index, inplace=True)
train_inte.drop(train_inte[train_inte["debt_loan_ratio"] > 42.88].index, inplace=True)
train_data.drop(train_data[train_data["house_exist"] > 2].index, inplace=True)
train_inte.drop(train_inte[train_inte["house_exist"] > 2].index, inplace=True)
train_data.drop(train_data[train_data["total_loan"] > 40000].index, inplace=True)
train_inte.drop(train_inte[train_inte["total_loan"] > 40000].index, inplace=True)
...
## 选择阈值0.07，从internet表中提取预测小于该概率的样本，并对不同来源的样本赋予来源值
InteId = IntePre.loc[IntePre.isDefault < 0.05 or IntePre.isDefault > 0.93, 'loan_id'].tolist()
...
print(f"时候了含有nan:{data.isna().sum()}")
...
def XGBModel(data_, test_, y_, folds_):
    ...
    for n_fold, (trn_idx, val_idx) in enumerate(folds_.split(data_)):
        ...
        clf = XGBClassifier(eval_metric='auc', max_depth=5, alpha=0.3, reg_lambda=0.3, subsample=0.8,
                            ...
                            learning_rate=0.01, n_estimators=6000, min_child_weight=2, tree_method='hist',
                            n_jobs=-1)
        clf.fit(trn_x, trn_y,
                eval_set=[(trn_x, trn_y), (val_x, val_y)],
                eval_metric='auc', verbose=100, early_stopping_rounds=100  # 40
                )
    ...
    #test_[[i for i in range[test_.shape[1]]], 'isDefault'] = sub_preds
    test_['isDefault'] = sub_preds
    return test_[['loan_id', 'isDefault']]
...
def LGBModel(data_, test_, y_, folds_):
    ...
    for n_fold, (trn_idx, val_idx) in enumerate(folds_.split(data_)):
        ...
        joblib.dump(clf, f'model{n_fold+1}.pkl')
       # clf.save_model(f'model{n_fold+1}.txt')
        oof_preds[val_idx] = clf.predict_proba(val_x, num_iteration=clf.best_iteration_)[:, 1]
    ...
    print('Full AUC score %.6f' % roc_auc_score(y_, oof_preds))
    test_.loc[:, 'isDefault'] = sub_preds
    #result=test_.copy()
    #test_.loc.__setitem__((slice(None), 'isDefault'), sub_preds)
    #test_.loc[:, 'isDefault'] = sub_preds
    return test_[['loan_id', 'isDefault']]
...
y = train['isDefault']
folds = KFold(n_splits=5, shuffle=True, random_state=546789)
test_preds = XGBModel(train, test, y, folds)
test_preds.rename({'loan_id': 'id'}, axis=1)[['id', 'isDefault']].to_csv('result_XGM_0.01.csv', index=None)